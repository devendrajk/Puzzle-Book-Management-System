...
const { readTokenFromFile, shouldReadTokenFromFile, } = require('../configuration/connection_configuration');
...
function ConnectionConfig(options, validateCredentials, qaMode, clientInfo) {
    ...
    if (shouldReadTokenFromFile(options) && options.tokenFilePath) {
        const token = readTokenFromFile(options.tokenFilePath);
        if (token) {
            options.token = token;
        }
        else {
            throw new Error(`Missing token value in ${options.tokenFilePath}`);
        }
    }
    ...
    this.browserResponseRenderer = options.browserResponseRenderer;
    ...
    this.serverSessionKeepAlive = options.serverSessionKeepAlive ?? false;

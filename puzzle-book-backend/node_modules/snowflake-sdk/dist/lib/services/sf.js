...
StateConnected.prototype.destroy = function (options) {
    // If serverSessionKeepAlive is true, skip session deletion on the server
    // This allows async queries to continue running after disconnect
    if (this.connectionConfig.serverSessionKeepAlive) {
        Logger.getInstance().debug('serverSessionKeepAlive is enabled, skipping session deletion on server');
        this.snowflakeService.transitionToDisconnected();
        options.callback();
        return;
    }

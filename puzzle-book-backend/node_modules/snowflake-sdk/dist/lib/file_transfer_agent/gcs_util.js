...
function GCSUtil(connectionConfig, httpClient) {
    ...
    this.uploadFileResumable = async function (dataFile, fileSize, meta, encryptionMetadata) {
        ...
        try {
            ...
            sessionUrl = initResp.headers?.location;
            if (!sessionUrl) {
            ...
            try {
                ...
                while (position < fileSize) {
                    ...
                    if (chunkResp.status === 308) {
                        // GCS reports the last committed byte in the Range header (e.g.
                        // "bytes=0-262143" means the next write starts at 262144). A missing
                        // or malformed header means we can't trust the committed offset, so we
                        // throw rather than risk skipping un-persisted bytes and corrupting the
                        // object.
                        const commitedRangeStr = chunkResp.headers?.range;
                        const parsedRange = commitedRangeStr && /^bytes=(\d+)-(\d+)$/.exec(commitedRangeStr);
                        if (!parsedRange) {
                            throw new Error(`GCS resumable 308 returned no usable Range header (got "${commitedRangeStr}"); cannot safely resume`);
                        }
                        position = parseInt(parsedRange[2], 10) + 1;
                    }
                    else {
                        // Last chunk is 200OK with no range header
                        position = fileSize;
                    }
                }
            }
            ...
            if (sessionUrl) {
                try {
                    await axios.delete(sessionUrl, { headers: { Authorization: `Bearer ${accessToken}` } });
                }
                catch (_cleanupErr) {
                    // Suppress.
                }
    ...
    this.uploadFileStream = async function (fileStream, meta, encryptionMetadata) {
        ...
        const gcsHeaders = {
            'Content-Encoding': normalizeGcsContentEncoding(meta),
            ...buildGcsEncryptionHeaders(meta, encryptionMetadata),
        };
        if (accessToken) {
